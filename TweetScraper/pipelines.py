...
class SaveToMongoPipeline(object):
    ...
    def __init__(self):
        ...
        self.tweetCollection.ensure_index(
            [('ID', pymongo.ASCENDING)], unique=True, dropDups=True)
        self.userCollection.ensure_index(
            [('ID', pymongo.ASCENDING)], unique=True, dropDups=True)
...
class SavetoMySQLPipeline(object):
    ...
    def find_one(self, trait, value):
        select_query = "SELECT " + trait + " FROM " + \
            self.table_name + " WHERE " + trait + " = " + value + ";"
        try:
    ...
    def insert_one(self, item):
        ...
        insert_query = 'INSERT INTO ' + self.table_name + \
            ' (ID, url, datetime, text, user_id, usernameTweet )'
...
class SaveToFilePipeline(object):
    ...
    def get_search_query(self, spider):
        # print(spider.query)
        return spider.query
